...
const useStyles = makeStyles((theme) => ({
  ...
  // Novos estilos para as listas ocultas
  hiddenTicketsList: {
    position: "absolute",
    top: -9999,
    left: -9999,
...
const TicketsManagerTabs = () => {
  ...
  // Estados para controlar notificações globais
  const [lastTicketCounts, setLastTicketCounts] = useState({
    open: 0,
    pending: 0,
    group: 0,
    closed: 0,
  ...
  // Função para verificar se houve mudança nos contadores e notificar
  const checkForNewTickets = (status, newCount, oldCount) => {
    if (newCount > oldCount && oldCount > 0) {
      ...
      switch (status) {
        ...
        case "group":
          ...
          break;
        case "closed":
          statusText = "Resolvidos";
          break;
        default:
          statusText = "Tickets";
      }

      const title = "Novo Ticket!";
      const body = `${difference} novo${difference > 1 ? "s" : ""} ticket${
        difference > 1 ? "s" : ""
      } em ${statusText}`;

      showBrowserNotification(title, body);

      // Também podemos fazer o título da página piscar
      const originalTitle = document.title;
      let isOriginal = true;
      const blinkInterval = setInterval(() => {
        document.title = isOriginal
          ? `(${difference}) Nova Mensagem!`
          : originalTitle;
        isOriginal = !isOriginal;
      }, 1000);

      // Parar de piscar após 10 segundos ou quando a janela ganhar foco
      const stopBlinking = () => {
  ...
  // Atualizar contadores e verificar por novos tickets
  const updateTicketCount = (status, count) => {
    setLastTicketCounts((prev) => {
      const oldCount = prev[status] || 0;

      // Verificar se houve aumento no número de tickets
      checkForNewTickets(status, count, oldCount);
    ...
    // Atualizar os estados específicos
    switch (status) {
      case "open":
        setOpenCount(count);
        break;
